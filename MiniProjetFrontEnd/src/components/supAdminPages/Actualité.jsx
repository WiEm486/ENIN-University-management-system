...
function Actualité() {
  ...
  const fetchActualite = () => {
    ActualitéService.getActualites()
      .then(data => {
        setActualites(data);
  ...
  const handleFiltreCibleChange = (e) => {
  ...
  const filtrerActualites = () => {
    ...
    // Filtrer par cible
    if (filtreCible !== '') {
      filtered = filtered.filter(actualite => actualite.cible === filtreCible);
    }
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                ...
                value={filtreCible}
                onChange={handleFiltreCibleChange}
                variant="outlined"
                fullWidth
              >
